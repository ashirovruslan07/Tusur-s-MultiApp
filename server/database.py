...
PK_FIELDS = {
    ...
    "Weekly_Schedule": "schedule_id",
    "Daily_Schedule": "daily_schedule_id",
    "Schedule_Weeks": "schedule_week_id",
    "Accounts": "account_id",
    ...
    "Plan_Exercises": "plan_exercise_id",
    "Workout_Logs": "workout_log_id",
    "Workout_Log_Exercises": "log_exercise_id",
    "Planner_Categories": "planner_category_id",
    "Tasks": "task_id",
    "Events": "event_id",
    "Notes": "note_id",
    "Portfolio_Categories": "portfolio_category_id",
    "Portfolio_Projects": "project_id",
    "Portfolio_Achievements": "achievement_id",
    "Portfolio_Certificates": "certificate_id",
    "Portfolio_Files": "file_id",
    "Portfolio_Skills": "skill_id",
    "Schedule_Sync_Log": "log_id",
}
...
def db_label() -> str:
    return DATABASE_URL or str(DB_PATH)
...
def _convert_sqlite_schema(sql: str) -> str:
...
def _convert_insert_or_ignore(sql: str) -> str:
    ...
    if not stripped.upper().startswith("INSERT OR IGNORE INTO "):
        return sql
    return re.sub(
...
def _unquote_postgres_identifiers(sql: str) -> str:
    return re.sub(r'"([A-Za-z_][A-Za-z0-9_]*)"', lambda match: match.group(1).lower(), sql)


def _returning_id_sql(sql: str) -> str:
    stripped = sql.strip().rstrip(";")
    if not stripped.upper().startswith("INSERT INTO ") or re.search(r"\bRETURNING\b", stripped, re.IGNORECASE):
        return sql
    match = re.match(r'\s*INSERT\s+INTO\s+"?([A-Za-z_][A-Za-z0-9_]*)"?\s+', stripped, re.IGNORECASE)
    if not match:
        return sql
    pk_field = PK_FIELDS.get(match.group(1))
    if not pk_field:
        return sql
    return f'{stripped} RETURNING "{pk_field}"'


def _prepare_postgres_sql(sql: str) -> str:
    converted = _convert_insert_or_ignore(sql)
    converted = _returning_id_sql(converted)
    converted = _unquote_postgres_identifiers(converted)
    return _convert_params(converted)
...
class PgConnection:
    ...
    def execute(self, sql: str, params: dict[str, Any] | None = None) -> PgCursor:
        prepared = _prepare_postgres_sql(sql)
        cursor = self.raw.execute(prepared, params or {})
        lastrowid = None
        if re.search(r"\bRETURNING\b", prepared, re.IGNORECASE):
            row = cursor.fetchone()
            if row:
                lastrowid = next(iter(row.values()))
            cursor = self.raw.cursor(row_factory=dict_row_factory())
    ...
    def executemany(self, sql: str, params_seq: Iterable[dict[str, Any]]) -> None:
        prepared = _convert_params(_unquote_postgres_identifiers(_convert_insert_or_ignore(sql)))
        self.raw.cursor().executemany(prepared, list(params_seq))

    def executescript(self, script: str) -> None:
        for statement in _split_sql_script(_convert_sqlite_schema(script)):
...
def connect():
    if IS_POSTGRES:
        ...
        return PgConnection(psycopg.connect(DATABASE_URL, row_factory=dict_row_factory()))
...
def monday_for_date(value: datetime.date) -> datetime.date:
    return value - timedelta(days=value.weekday())


def backfill_schedule_weeks(conn: sqlite3.Connection) -> None:
    schedules = conn.execute(
        """
        SELECT schedule_id, source_week_id, week_number, week_type, starts_at, synced_at, schedule_week_id
        FROM Weekly_Schedule
        """
    ).fetchall()
    for schedule in schedules:
        starts_at = parse_iso_date(schedule.get("starts_at"))
        if starts_at is None:
            synced_date = parse_iso_date(schedule.get("synced_at")) or datetime.now(APP_TZ).date()
            starts_at = monday_for_date(synced_date)
            conn.execute(
                "UPDATE Weekly_Schedule SET starts_at = :starts_at WHERE schedule_id = :schedule_id",
                {"starts_at": starts_at.isoformat(), "schedule_id": schedule["schedule_id"]},
            )

        ends_at = starts_at + timedelta(days=6)
        week_type = schedule.get("week_type") or "обычная"
        params = {
            "source_week_id": schedule.get("source_week_id"),
            "week_number": schedule.get("week_number"),
            "week_type": week_type,
            "starts_at": starts_at.isoformat(),
            "ends_at": ends_at.isoformat(),
            "synced_at": schedule.get("synced_at") or datetime.now(APP_TZ).isoformat(timespec="seconds"),
        }
        conn.execute(
            """
        ...
        if params["source_week_id"] is not None:
            week_row = conn.execute(
                "SELECT schedule_week_id FROM Schedule_Weeks WHERE source_week_id = :source_week_id",
        ...
        if week_row and schedule.get("schedule_week_id") != week_row["schedule_week_id"]:
            conn.execute(
                "UPDATE Weekly_Schedule SET schedule_week_id = :schedule_week_id WHERE schedule_id = :schedule_id",
                {"schedule_week_id": week_row["schedule_week_id"], "schedule_id": schedule["schedule_id"]},
            )
...
def migrate(conn: sqlite3.Connection) -> None:
    ...
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS Schedule_Weeks (
            schedule_week_id INTEGER PRIMARY KEY AUTOINCREMENT,
            source_week_id INTEGER,
            week_number INTEGER,
            week_type TEXT NOT NULL DEFAULT 'обычная'
                CHECK (week_type IN ('четная', 'нечетная', 'обычная', 'числитель', 'знаменатель')),
    ...
    add_column(conn, "Weekly_Schedule", "schedule_week_id", "schedule_week_id INTEGER")
    ...
    conn.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_weekly_schedule_week_id
            ON Weekly_Schedule(schedule_week_id);
        CREATE UNIQUE INDEX IF NOT EXISTS idx_schedule_weeks_source_week_id
            ON Schedule_Weeks(source_week_id) WHERE source_week_id IS NOT NULL;
        CREATE UNIQUE INDEX IF NOT EXISTS idx_schedule_weeks_starts_type
            ON Schedule_Weeks(starts_at, week_type);
...
def get_user_by_session(session_id: str | None) -> dict[str, Any] | None:
    ...
    return fetch_one(
        ...
        """,
        {"session_id": session_id, "now": datetime.now(APP_TZ).isoformat(timespec="seconds")},
    )
